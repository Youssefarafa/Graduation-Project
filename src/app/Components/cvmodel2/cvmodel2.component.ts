...
@Component({
  selector: 'app-cvmodel2',
  standalone: true,
  imports: [NgIf, NgClass, RouterLink],
  templateUrl: './cvmodel2.component.html',
  styleUrl: './cvmodel2.component.scss',
})
export class CVModel2Component implements OnInit, OnDestroy {
    ...
    if (this.platformDetectionService.isBrowser) {
      ...
      this.platformDetectionService.executeAfterDOMRender(() => {
        this.ngZone.runOutsideAngular(() => {
          const endTime = Number(localStorage.getItem('video_timer_end2'));
  ...
  uploadImage(file: File): void {
    ...
    this.loading = true;
    this.isStreaming = true;
    this._AIService.getCVImagePrediction2(formData).subscribe({
      next: (res) => {
        // Start polling using the file name returned by backend
        this._AIService.pollForProcessedImageCV2(file).subscribe({
          next: (blob: Blob) => {
  ...
  startTimer(): void {
    ...
    localStorage.setItem('video_timer_end2', endTime.toString());
  ...
  updateCountdown(): void {
    const endTime = Number(localStorage.getItem('video_timer_end2') || 0);
    const timeLeft = Math.max(0, Math.ceil((endTime - Date.now()) / 1000));
    this.countdown = timeLeft;
    if (timeLeft <= 0) {
      this.stopTimer();
      this.isSelectedFileVideo = false;
      localStorage.removeItem('video_timer_end2');
    }
  }
  ...
  uploadVideo(file: File): void {
    ...
    this.loading = true;
    this.isStreaming = true;
    this._AIService.getCVVideoPredection2(formData).subscribe({
      next: (res: any) => {
        this._AIService.pollForProcessedVideoCV2(file).subscribe({
          next: (res: Blob) => {
  ...
  connectToWebSocket(): void {
    this.ws = new WebSocket('ws://localhost:5001/plants/PestDetection/predict-frame');
    // this.ws = new WebSocket('ws://localhost:5001/PestDetection/predict-frame');
    //!! this.ws = new WebSocket('wss://localhost:5001/PestDetection/predict-frame');
    //?? this.ws = new WebSocket("wss://definitely-normal-viper.ngrok-free.app/PestDetection/predict-frame");
